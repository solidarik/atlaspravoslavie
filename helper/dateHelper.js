...
export default class DateHelper {
    ...
    static getDateFromInput(input, stopWords = []) {
        ...
        if (!isFound && inputText.includes('в')) {
            ...
            if (!StrHelper.isExistNumber(input)) {
                ...
                date_groups = StrHelper.getSearchGroupsInRegexp(
                    '[-]*(\\S+)[в]',
                    inputText
                )
    ...
    static convertTZ(date, tzString) {
        ...
        ).toLocaleString('ru-RU', { timeZone: tzString })
    }
    ...
    static dateToStr(inputDate, isWithoutYear = true) {
        ...
        return isWithoutYear && day == '01' && month == '01'
            ? year
            : `${day}.${month}.${year} `
    }
    ...
    static getCenturyRange(century) {
        ...
        if (isMinus) return [-endYear, -startYear]
        else return [startYear, endYear]
    }

    static getWorshipDates(input) {
        let worshipDates = []
        if (!input) {
            return worshipDates
        }
        ...
        input = StrHelper.ignoreEqualsValue(input)
        let prevWordPos = 0
        ...
        worshipDates.forEach((worship) => {
            ...
            worships.push({
                day: day,
                month: month,
                dateStr: `${Number(day)} ${DateHelper.getInducementTextOfMonth(
                    month
                )}`,
            })
        })
