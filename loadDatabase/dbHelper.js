...
export default class DbHelper {
    ...
    async clearModel(model) {
        try {
            ...
            this.log.info(
                chalk.gray(
                    `Удаление коллекции: ${model.collection.collectionName}`
                )
            )
            return true
        } catch (err) {
    ...
    async clearDb(filter = '') {
        ...
        for (const modelFilePath of modelFiles) {
            if ('' == filter || modelFilePath.includes(filter)) {
                promises.push(
                    (async () => {
                        const modelFilePathFixed = 'file:///' + modelFilePath.replace(/[\\]+/g, '/')
                        const model = await import(modelFilePathFixed)
                        await model.default.deleteMany({})
                        this.log.info(
                            chalk.gray(
                                `Удаление коллекции: ${modelFilePath}`
                            )
                        )
                        return true
                    })()
    ...
    saveFilesFrom(input) {
        return new Promise((resolve, reject) => {
            ...
            this.log.info(
                `Начинаем обрабатывать ${dataTypeStr} ${chalk.cyan(
                    input.source
                )}`
            )
            ...
            this.log.info(
                `Количество входящих элементов, промисов: ${promises.length}`
            )
